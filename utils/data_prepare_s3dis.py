...
# Use normalized path for the dataset
dataset_path = normpath('./data/S3DIS/Stanford3dDataset_v1.2_Aligned_Version')
anno_paths = [line.rstrip() for line in open(join(BASE_DIR, 'meta/anno_paths.txt'))]
anno_paths = [normpath(join(dataset_path, p)) for p in anno_paths]
...
def convert_pc2ply(anno_path, save_path):
    ...
    for f in glob.glob(join(anno_path, '*.txt')):
        class_name = basename(f).split('_')[0]
    ...
    # Use basename to extract the file name without directories
    sub_ply_file = join(sub_pc_folder, basename(save_path)[:-4] + '.ply')
    write_ply(sub_ply_file, [sub_xyz, sub_colors, sub_labels], ['x', 'y', 'z', 'red', 'green', 'blue', 'class'])

    search_tree = KDTree(sub_xyz)
    kd_tree_file = join(sub_pc_folder, basename(save_path)[:-4] + '_KDTree.pkl')
    with open(kd_tree_file, 'wb') as f:
        pickle.dump(search_tree, f)

    proj_idx = np.squeeze(search_tree.query(xyz, return_distance=False))
    proj_idx = proj_idx.astype(np.int32)
    proj_save = join(sub_pc_folder, basename(save_path)[:-4] + '_proj.pkl')
...
if __name__ == '__main__':
    ...
    for annotation_path in anno_paths:
        annotation_path = normpath(annotation_path)
        print(annotation_path)
        # Instead of splitting on '/', use os.path.dirname and os.path.basename
        # Get the parent and grandparent directory names:
        parent_dir = basename(dirname(annotation_path))  # e.g., 'conferenceRoom_1'
        grandparent_dir = basename(dirname(dirname(annotation_path)))  # e.g., 'Area_1'
        out_file_name = grandparent_dir + '_' + parent_dir + out_format
