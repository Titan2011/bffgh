...
class ConfigSemantic3D:
    ...
    # noise_init = 1.0  # noise initial parameter
    # max_epoch = 100  # maximum epoch during training
    # learning_rate = 1e-3  # initial learning rate
    # lr_decays = {i: 0.97 for i in range(0, 100)}  # decay rate of learning rate
    ...
    # sub_sampling_ratio = [1,3,4,4,2]
    # d_out = [32, 64, 128, 256, 256]

    # noise_init = 1.0
    # max_epoch         = 150
    # learning_rate     = 5e-4
    # lr_decays         = {i: 0.98 for i in range(150)}
    ...
    experiment_name = "Semantic3D_Boundary"
    ...
    # ----------------------------------------------------------------------------
    # Path to the dataset
    data_path = "data/Semantic3D"
    
    # Ignored label index for loss function
    # In Semantic3D, the "unlabeled" class is typically 0
    ignored_label_inds = [0]

    # Number of classes in the dataset (including the ignored one if it's not remapped)
    num_classes = 8  # Number of valid classes for evaluation

    # Number of input features (e.g., x, y, z, r, g, b)
    num_features = 6
    
    # Size of the sub-grid for data preprocessing
    sub_grid_size = 0.06
    ...
    # Feature dimension for each layer
    d_out = [32, 64, 128, 256, 256]

    # Sub-sampling ratio for each layer
    sub_sampling_ratio = [1, 3, 4, 4, 2]
    ...
    lr_decays = {i: 0.95 for i in range(max_epoch)}
    ...
    # Number of points per input cloud
    num_points = 32768
    ...
    # Directory for training summary
    train_sum_dir = 'train_log'
    ...
    # ----------------------------------------------------------------------------
    # --- Boundary-aware settings
    # ----------------------------------------------------------------------------
    # Number of neighbors to consider for boundary detection
    boundary_k = 20
