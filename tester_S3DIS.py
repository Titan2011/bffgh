...
class ModelTester:
    ...
    def test(self, model, dataset, num_votes=100):
        ...
        # Build saving paths using os.path.join to avoid separator issues.
        saving_folder = time.strftime('Log_%Y-%m-%d_%H-%M-%S', time.gmtime())
        saving_path = normpath(join('results', saving_folder))
        test_path = normpath(join('test', saving_folder))
        makedirs(test_path, exist_ok=True)
        makedirs(join(test_path, 'val_preds'), exist_ok=True)
        ...
        while last_min < num_votes:
            try:
                ...
                if last_min + 1 < new_min:
                    ...
                    # Show vote results (on subcloud so it is not the final values)
