...
class Network: 
     ...
     def inference(self, inputs, is_training): 
         ...
         feature = tf.keras.layers.Dense(8, activation=None, name='fc0')(feature) 

         # Use tf.keras.layers.BatchNormalization instead of tf.layers.batch_normalization 

         bn_layer = tf.keras.layers.BatchNormalization(momentum=0.99, epsilon=1e-6) 



         # Use tf.cond to create conditional logic in the graph 

         # This tells TensorFlow to use training=True when the placeholder is True, and False otherwise 

         feature = tf.cond( 

             is_training, 

             lambda: bn_layer(feature, training=True), 

             lambda: bn_layer(feature, training=False) 
     ...
     def att_pooling(feature_set, d_out, name, is_training): 
         ...
         att_activation = tf.keras.layers.Dense(d, activation=None, use_bias=False, name=name + 'fc')(f_reshaped) 
